...
const DoctorsPage = () => {
  ...
  const columns = [
    { key: 'id',             label: t('common.id',   { ns: 'common' }) },
    { key: 'name',           label: t('common.name', { ns: 'common' }), render: (r) => r.profile?.full_name ?? '—' },
    { key: 'specialization', label: t('nav.specializations', { ns: 'common' }), render: (r) => r.work_configuration?.specialization?.name ?? '—' },
    { key: 'facility',       label: t('nav.facilities', { ns: 'common' }), render: (r) => r.work_configuration?.facility_department?.facility?.name ?? '—' },
    { key: 'phone',          label: t('common.phone', { ns: 'common' }), render: (r) => r.profile?.phone ?? '—' },
  ];

  const fields = [
    { name: 'facility_department_id', label: t('nav.facilities', { ns: 'common' }), type: 'select', options: facilities },
    { name: 'specialization_id',      label: t('nav.specializations', { ns: 'common' }), type: 'select', options: specs },
  ];
  ...
  return (
    <CrudPage
      ...
      isLoading={isLoading}
      fields={fields}
      initialValues={{ facility_department_id: '', specialization_id: '' }}
      onCreate={(v) => createMut.mutateAsync({
        work_configuration: {
          facility_department_id: v.facility_department_id,
